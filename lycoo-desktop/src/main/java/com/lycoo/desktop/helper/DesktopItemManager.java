...
//                                    .delay(5, TimeUnit.SECONDS) // for test prepareing
    ...
    public void onPackageChanged(Intent intent,String image) {
        String action = intent.getAction();
        LogUtils.debug(TAG, "action : " + action);
        if (!StringUtils.isEmpty(action)) {
            String data = intent.getDataString();
            if (!StringUtils.isEmpty(data)) {
                ...
                // 升级（也可以不做处理，纯粹为了用户体验（因为有的应用升级之后label和icon可能发生变化，让用户第一时间看到））
                    ...
                    onAppUpdateData(packageName);
                    onKTVAppUpdateData(packageName,image);
     ...
     * @param packageName
     */
    private void onKTVAppUpdateData(String packageName,String image) {
